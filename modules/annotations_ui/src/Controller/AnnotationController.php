 ...
 *
 * Add page: shows a unified table of annotation slots (overview and per-field)
 * with inline Add links per missing type. When all slots are filled, a link to
 * the edit view is shown instead.
 *
...
class AnnotationController extends ControllerBase {
  ...
  public function addPage(AnnotationTarget $annotation_target, Request $request): array {
    ...
    $build['fields_table'] = [
      '#theme' => 'table',
      '#header' => [$this->t('Scope'), $this->t('Annotations')],
      '#rows' => $rows,
    ];
    return $build;
  }
